...
class FileCleanupCommand extends Command
{
    ...
    public function handle()
    {
        ...
        if (! File::exists($jsonFilePath)) {
            ...
            Log::error("JSON file not found at: {$jsonFilePath}");
        ...
        Log::info("Reading file list from: {$jsonFilePath}");

        try {
            $filesList = json_decode(File::get($jsonFilePath), true);
            if (json_last_error() !== JSON_ERROR_NONE) {
                $this->error('Invalid JSON file: ' . json_last_error_msg());
                Log::error('Invalid JSON file: ' . json_last_error_msg());
            ...
            foreach ($filesList as $filePath) {
                ...
                if (File::exists(base_path($filePath))) {
                    try {
                        Log::alert("Deleting file: {$filePath}");
