...
class WhatsAppWebhookController extends Controller
{
            ...
            foreach ($message_data as $data) {
                try {
                    switch ($data['type']) {
                        ...
                        case 'audio':
                            $audio_url = $this->sanitizeMediaUrl($data['audio']['url']);
                            $response = $whatsapp_cloud_api->sendAudio($to, new LinkID($audio_url));
                            break;
                    
                        case 'image':
                            $image_url = $this->sanitizeMediaUrl($data['image']['url']);
                            $response = $whatsapp_cloud_api->sendImage($to, new LinkID($image_url));
                            break;
                    
                        case 'video':
                            $video_url = $this->sanitizeMediaUrl($data['video']['url']);
                            $response = $whatsapp_cloud_api->sendVideo($to, new LinkID($video_url));
                            break;
                    
                        case 'document':
                            $document_url = $this->sanitizeMediaUrl($data['document']['url']);
                            $fileName = basename($document_url);
                            $response = $whatsapp_cloud_api->sendDocument($to, new LinkID($document_url), $fileName, '');
                            break;
    ...
    private function sanitizeMediaUrl($url) {
