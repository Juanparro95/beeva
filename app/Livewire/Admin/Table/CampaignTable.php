...
final class CampaignTable extends PowerGridComponent
{
    ...
    public function datasource(): Builder
    {
        ...
        if (checkPermission('contact.view')) {   
            ...
            // User only has view_own permission, filter to show only campaigns with contacts assigned to them
            $staffId = auth()->id();
            
            // Subquery to find campaigns with contacts assigned to the current staff
            return $query->whereExists(function ($query) use ($staffId) {
                $query->select(DB::raw(1))
                    ->from('campaign_details')
                    ->join('contacts', 'campaign_details.rel_id', '=', 'contacts.id')
                    ->whereColumn('campaign_details.campaign_id', 'campaigns.id')
                    ->where('contacts.assigned_id', $staffId);
            });
        }
    }
